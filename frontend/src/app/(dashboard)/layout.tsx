...
// `soon` items render as disabled rows rather than links — the pages don't
// exist yet, and a nav entry that 404s is worse than one that says "soon".
const NAV_MAIN = [
...
function NavLink({
  ...
  if (soon) {
    return (
      <div
        className="flex cursor-default items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium"
        style={{ color: "#3D3D48", border: "1px solid transparent" }}
      >
        <Icon className="h-4 w-4 shrink-0" />
        <span className="flex-1">{label}</span>
        <span
          className="rounded-full px-1.5 py-0.5 text-[9px] font-medium"
          style={{ background: "#18181E", border: "1px solid #2A2A34", color: "#555560" }}
        >
          Soon
        </span>
      </div>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span className="hidden items-center gap-1.5 text-xs sm:flex" style={{ color: "#888896" }}>
              ...
              >
                Synced{" "}
                {minutesSinceLoad === 0 ? "just now" : `${minutesSinceLoad} min ago`}
              </span>
            </span>
            <button
              onClick={handleSync}
