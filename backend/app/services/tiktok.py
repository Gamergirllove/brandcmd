...
class TikTokService(BasePlatformService):
    ...
    The PKCE verifier is supplied by the caller and stored server-side
    with the CSRF state, so the flow survives a restart or a second
    worker process picking up the callback.
    """

    requires_pkce = True

    async def get_auth_url(self, state: str, code_verifier: Optional[str] = None) -> str:
    ...
    async def exchange_code(
        ...
        Args:
            ...
            code_verifier: The PKCE verifier that produced the challenge sent
                           to the authorize endpoint.
        """
        if not code_verifier:
            raise PlatformAuthError(
                "TikTok requires a PKCE code_verifier — the one generated for "
                "this authorization's state was not supplied."
            )
        payload = {
            "client_key": self.client_id,
