...
class AnalyticsData:
    ...
    # Platform-specific metrics that don't fit the common shape —
    # e.g. YouTube watch time, Twitch subscriber count and avg viewers.
    # Surfaced verbatim as PlatformStats.raw.
...
class BasePlatformService(abc.ABC):
    ...
    Subclasses whose provider requires PKCE set ``requires_pkce = True``;
    the caller then generates the verifier, persists it alongside the CSRF
    state, and passes it to both ``get_auth_url`` and ``exchange_code``.
    """

    #: Whether this provider requires a PKCE code_verifier/code_challenge pair.
