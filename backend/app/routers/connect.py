...
import httpx
import urllib.parse
from app.services.pkce import generate_code_verifier, generate_code_challenge
...
OAUTH_CONFIGS = {
    ...
    "twitter": {
        ...
        "extra_params": {},  # PKCE params injected dynamically in get_oauth_url
        "uses_pkce": True,
    },
    "pinterest": {
...
async def get_oauth_url(platform: str, user_id: str = Depends(get_current_user)):
    ...
    # Inject PKCE for platforms that require it (e.g. Twitter)
    code_verifier = None
    if config.get("uses_pkce"):
        ...
        # Embed verifier in state so callback can use it: user_id:state:verifier
        params["state"] = f"{user_id}:{state}:{code_verifier}"
...
async def oauth_callback(
    ...
    user_id = parts[0]
    # For PKCE platforms, verifier is embedded as 3rd segment: user_id:nonce:verifier
    code_verifier = parts[2] if len(parts) >= 3 else None
    ...
    # Include PKCE verifier for platforms that require it
    if code_verifier and config.get("uses_pkce"):
        token_post_data["code_verifier"] = code_verifier
